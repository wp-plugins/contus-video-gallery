...
<?php
        ...
        $div = '<div id="videos-search"  class="sidebar-wrap clearfix">';
        $div .= '<form role="search" method="POST" id="searchform" action="' . home_url('/') . '?page_id=' . $moreName . '" >
	<div><label class="screen-reader-text" for="s">' . __('Search for:') . '</label>
	<input type="text" value="' . $searchVal . '" ' . $focusVal . $blurVal . ' name="video_search" id="video_search"  />
	<input type="submit" id="searchsubmit" value="' . __('Search', 'video_gallery') . '" />
	</div>
	</form>';
        $div .='</div>';
        echo $div;
